...
def kitaev_ladder(L, J_ray, h_ray, BC='periodic'):
    ...
    H_zz = [[Jz,i,L + i] for i in range(L)] # PBC
    H_xx_1 = [[Jx,2*i,(2*i+1) % (L-1)] for i in range(L//2 + BC * (L%2))] # PBC
    H_xx_2 = [[Jx,L + 2*i + 1,L + (2*i+2)%L] for i in range(L//2 + BC * (1 - L%2) - 1*(1 - L%2))] # PBC
    H_yy_1 = [[Jy,2*i+1,(2*i+2) % (L-1)] for i in range(L//2 + BC * (1 - L%2) - 1*(1 - L%2))] # PBC
    H_yy_2 = [[Jy,L + 2*i, L + (2*i+1)%L] for i in range(LL//2 + BC * (L%2))] # PBC
    H_z = [[J_x,i] for i in range(2*L)] # PBC
    H_y = [[J_x,i] for i in range(2*L)] # PBC
    H_x = [[J_x,i] for i in range(2*L)] # PBC
